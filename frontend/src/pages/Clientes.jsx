...
export default function Clientes() {
  ...
  return (
    <div className="clientes-container animate-fade-in" style={{ display: 'flex', gap: '1.5rem', height: 'calc(100vh - 100px)' }}>
      
      {/* Panel Izquierdo: Lista de Clientes */}
      <div className="clientes-list" style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: '1rem', minWidth: 0 }}>
        <header>
        ...
        <div style={{ overflowY: 'auto', paddingRight: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {filtrados.map(cliente => (
            <div key={cliente.id} className="glass-panel cliente-card" style={{ padding: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <h3 style={{ fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: '0.5rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '0.2rem' }}>CC/NIT: {cliente.documento_identidad || 'N/A'}</p>
                </div>
              </div>
              
              <div className="cliente-contact" style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.25rem', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      <div className="glass-panel clientes-form" style={{ flex: 1, padding: '1.5rem', minWidth: '340px', display: 'flex', flexDirection: 'column', height: 'fit-content' }}>
        ...
        <form onSubmit={handleCrearCliente} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="input-group" style={{ marginBottom: 0 }}>
            ...
            <select name="tipo_cliente" className="input-field" value={formData.tipo_cliente} onChange={handleInputChange}>
      ...
      <style>{`
        @media (max-width: 900px) {
          .clientes-container { flex-direction: column !important; height: auto !important; }
          .clientes-form { min-width: 100% !important; order: -1; }
          .cliente-card { flex-direction: column !important; align-items: flex-start !important; }
          .cliente-contact { text-align: left !important; }
          .cliente-contact p { justify-content: flex-start !important; }
        }
      `}</style>
    </div>
  );
